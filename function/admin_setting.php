...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($full_name) || empty($username) || empty($email)) {
        $_SESSION['error'] = "Full name, username, and recovery email cannot be empty.";
    ...
    try {
        ...
        $check_username = $pdo->prepare("SELECT id FROM admin WHERE username = ? AND id != ? LIMIT 1");
        $check_username->execute([$username, $admin_id]);
        if ($check_username->fetch()) {
            throw new Exception("This username is already taken by another account.");
        }
        ...
        $update_info = $pdo->prepare("UPDATE admin SET name = ?, username = ?, email = ? WHERE id = ?");
        $update_info->execute([$full_name, $username, $email, $admin_id]);
